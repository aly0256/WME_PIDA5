Public Class frmEstacionesTrabajo
...
#End Region
    ...
    Private Sub frmEstacionesTrabajo_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            dtLista = sqlExecute("SELECT cod_estacion as 'Código',Nombre FROM estaciones_trabajo", "IDEAS")
            dtLista.DefaultView.Sort = "Código"
            dgTabla.DataSource = dtLista
            dgTabla.Columns(1).AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill

            dtRegistro = sqlExecute("SELECT TOP 1 * FROM estaciones_trabajo ORDER BY cod_estacion ASC ", "IDEAS")
            MostrarInformacion()
    ...
    Private Sub MostrarInformacion()
        ...
        Try
            ...
            txtCodigo.Text = dtRegistro.Rows(0).Item("cod_estacion")
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        Dim Cod As String
        Cod = Buscar("ideas.dbo.estaciones_trabajo", "cod_estacion", "ESTACIONES DE TRABAJO", False)
        If Cod <> "CANCELAR" Then
            dtRegistro = sqlExecute("SELECT * FROM estaciones_trabajo WHERE cod_estacion = '" & Cod & "' ", "IDEAS")
            MostrarInformacion()
        End If
    End Sub

    Private Sub btnPrimero_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrimero.Click
        Primero("estaciones_trabajo", "cod_estacion", dtregistro, "IDEAS")
        MostrarInformacion()
    End Sub

    Private Sub btnCiasPrev_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAnterior.Click
        Anterior("estaciones_trabajo", "cod_estacion", txtCodigo.Text, dtregistro, "IDEAS")
        MostrarInformacion()
    End Sub

    Private Sub btnSiguiente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSiguiente.Click
        Siguiente("estaciones_trabajo", "cod_estacion", txtCodigo.Text, dtregistro, "IDEAS")
        MostrarInformacion()
    End Sub

    Private Sub btnBorrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBorrar.Click
        Codigo = txtCodigo.Text
        dtTemporal = sqlExecute("SELECT TOP 1 FROM ideas_empleado WHERE cod_estacion = '" & Codigo & "'", "IDEAS")
        ...
        Else
            If MessageBox.Show("¿Está seguro de borrar el registro " & Codigo & "?", "Borrar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                sqlExecute("DELETE FROM estaciones_trabajo WHERE cod_estacion = '" & Codigo & "'", "IDEAS")
    ...
    Private Sub dgTabla_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgTabla.RowEnter
        ...
        dtRegistro = sqlExecute("SELECT * FROM estaciones_trabajo WHERE cod_estacion = '" & cod & "' AND nombre = '" & nom & "'", "IDEAS")
        MostrarInformacion()
    End Sub
    ...
    Private Sub btnUltimo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUltimo.Click
        Ultimo("estaciones_trabajo", "cod_estacion", dtregistro, "IDEAS")
        MostrarInformacion()
    End Sub
    ...
    Private Sub btnNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNuevo.Click
        ...
        If Agregar Then
            ' Si Agregar, revisar si existe cod_estacion
            dtTemporal = sqlExecute("SELECT cod_estacion FROM estaciones_trabajo where cod_estacion = '" & txtCodigo.Text & "'", "IDEAS")
            If dtTemporal.Rows.Count > 0 Then
                MessageBox.Show("El registro no se puede agregar, ya existe '" & txtCodigo.Text & "'.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                txtCodigo.Focus()
                Exit Sub
            Else
                sqlExecute("INSERT INTO estaciones_trabajo (cod_estacion,nombre) VALUES ('" & txtCodigo.Text & "','" & txtNombre.Text & "')", "IDEAS")
                Agregar = False
            End If

        ElseIf Editar Then
            ' Si Editar, entonces guardar cambios a registro
            sqlExecute("UPDATE estaciones_trabajo SET nombre = '" & txtNombre.Text & "' WHERE cod_estacion = '" & txtCodigo.Text & "'", "IDEAS")
        Else
            Agregar = True
        End If
        Editar = False
