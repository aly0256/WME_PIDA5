Public Class frmAgregarConsultaFam
    ...
    Private Sub Agendar()
        Try
            Dim inicio As Date = inicio_consulta
            Dim cod_servicio As String = servicio_consulta
            Dim familiar As Integer = True
    ...
    Private Sub ButtonX2_Click(sender As Object, e As EventArgs) Handles btnAceptar.Click
        Try
            ...
            Dim dtMayores As DataTable = sqlExecute("SELECT * FROM familiares where idfld = '" & fam & "' and (cod_familia=08 or cod_familia=07) and substring(cast(fecha_nac as VARCHAR),1,4)<='1999' order by fecha_nac asc")
            If dtMayores.Rows.Count = 0 Then Agendar()
            If dtMayores.Rows.Count > 0 Then
    ...
    Private Sub frmAgregarConsulta_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            comboFamiliaresRegistrados.DataSource = sqlExecute("select familiares.idfld as 'ID', familia.cod_familia as 'Código',familia.nombre as 'Parentesco',RTrim(familiares.nombre) as 'Nombre' from familiares left join familia on familiares.cod_familia=familia.cod_familia where reloj = '" & reloj & "' AND familiares.idfld='" & fam & "'")
        Catch ex As Exception
            ErrorLog(Usuario, System.Reflection.MethodBase.GetCurrentMethod.Name(), Me.Name, ex.HResult, ex.Message)
        End Try
    End Sub
        ...
        Try
            If dtPersonal.Rows.Count > 0 Then
                ...
                Try
                    ...
                    'comboFamiliaresRegistrados.SelectedValue = fam
                    comboFamiliaresRegistrados.DataSource = sqlExecute("select familiares.idfld as 'ID', familia.cod_familia as 'Código',familia.nombre as 'Parentesco',RTrim(familiares.nombre) as 'Nombre' from familiares left join familia on familiares.cod_familia=familia.cod_familia where reloj = '" & txtReloj.Text & "' AND familiares.idfld='" & fam & "'")
    ...
    Private Sub cmbsintoma_SelectedValueChanged(sender As Object, e As EventArgs) Handles cmbsintoma.SelectedValueChanged
        Try
            ...
            Else
                If dtdesc.Columns(0).ColumnName <> "ERROR" Then
                    cmbdescripcion.DataSource = dtdesc
                End If
            End If
        Catch ex As Exception
