...
Public Class frmEntrevistaRH
    ...
    Private Sub frmEvaluacionSolicitantes_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            txtVacante.Text = ""
            Campos = "Folio, rtrim(nombre)+' '+rtrim(SegundoNombre)+' '+rtrim(Paterno)+' '+rtrim(Materno) As nombres,cod_vac,FhaApli,ComentariosEntrevistaRH,AprobadoEntrevistaRH,DuracionEntrevistaRH"
        ...
        Try
            ...
            If dtPersonal.Rows.Count > 0 Then
                With dtPersonal.Rows.Item(0)
                    ...
                    If IsDBNull(.Item("AprobadoEntrevistaRH")) Then
                        swAprobado.CheckState = CheckState.Indeterminate
                        swAprobado.Text = "Pendiente"
                    ElseIf .Item("AprobadoEntrevistaRH") = 1 Then
                    ...
                    End If
                    txtComentariosSupervisor.Text = IIf(IsDBNull(.Item("ComentariosEntrevistaRH")), "", .Item("ComentariosEntrevistaRH"))
                    clkCronometro.Value = IIf(IsDBNull(.Item("DuracionEntrevistaRH")), "00:00:00", .Item("DuracionEntrevistaRH").ToString)
    ...
    Private Sub CrearEvaluaciones()
        Try
            ...
            dtEvaluaciones = sqlExecute("SELECT DISTINCT evaluaciones_solicitantes.cod_evaluacion,nombre,cuestionarios,entrevistador " & _
                                        "FROM evaluaciones_solicitantes INNER JOIN evaluaciones " & _
                                        "ON evaluaciones_solicitantes.cod_evaluacion = evaluaciones.cod_evaluacion and evaluaciones.filtro = 'Entrevista RH' " & _
    ...
    Private Function AgregaEvaluacion() As DevComponents.AdvTree.Node
        ...
        Try
            ...
            If frmAgregarEvaluacionReclutamiento.ShowDialog("Entrevista RH") = Windows.Forms.DialogResult.OK Then
                Dim sdNodo As DevComponents.AdvTree.Node
                CodEval = strRespuesta

                dtEvaluacion = sqlExecute("SELECT cod_evaluacion,nombre,cuestionarios FROM evaluaciones WHERE cod_evaluacion = '" & CodEval & "'", "RECLUTAMIENTO")
                If dtEvaluacion.Rows.Count > 0 Then
                    ...
                    For Each C In ArCuestionarios
                        ...
                        sqlExecute("INSERT INTO evaluaciones_solicitantes (folio,cod_evaluacion,cod_cuestionario)" & _
                                   "VALUES ('" & txtFolio.Text & "','" & CodEval & "','" & C & "')", "RECLUTAMIENTO")
                    Next
    ...
    Private Function evaluar() As Boolean
        ...
        Try
            ...
            If preguntas > respuestas Then
                Return False
            Else
                strQry = "Update Candidatos set AprobadoEntrevistaRH = '" & IIf(swAprobado.CheckValue, "1", "0") & "', ComentariosEntrevistaRH = '" & txtComentariosSupervisor.Text.Trim & "' where Folio = '" & txtFolio.Text.Trim & "' "
