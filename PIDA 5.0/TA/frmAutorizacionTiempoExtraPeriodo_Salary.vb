Public Class frmAutorizacionTiempoExtraPeriodo_Salary
    ...
    Private Sub frmAutorizacionTiempoExtraPeriodo_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            Dim dtPeriodos As DataTable
                ...
                Me.Text = "Resumen semanal TA & TE Salary"
    ...
    Public Sub carga_estructura(Optional VerPrimerSupervisor As Boolean = True)
        ...
        Try
            ...
            dtPeriodo = sqlExecute("select * from ta.dbo.periodos where ano+periodo = '" & anoper & "'")
            ...
            If dtPeriodo.Rows.Count > 0 Then
                ...
                If fini >= Date.Today Then
                    dtMiPersonal = ConsultaPersonalVW("select reloj,cod_comp, nombres, cod_planta, cod_turno, cod_hora, cod_depto, cod_clase, isnull(cod_super, '') as cod_super, isnull(nombre_super, 'Sin Supervisor') as nombre_super from personalvw where cod_tipo = 'A' AND alta <= '" & FechaSQL(ffin) & "' and (baja is null or baja >= '" & FechaSQL(fini) & "')   and isnull(cod_super, '') <> '' and cod_planta = '" & IIf(sbPlanta.Value, sbPlanta.ValueTrue, sbPlanta.ValueFalse) & "' order by reloj", False)
                Else

                    dtMiPersonal = sqlExecute("select distinct reloj,cod_comp, nombres, cod_planta, cod_turno,cod_hora, cod_depto, cod_clase, isnull(cod_super, '') as cod_super, isnull(nombre_super, 'Sin Supervisor') as nombre_super from tavw " & _
                                              "where cod_tipo = 'A' AND " & IIf(FiltroXUsuario.Length > 0, FiltroXUsuario & " and ", "") & " FHA_ENT_HOR between '" & FechaSQL(fini) & "' and '" & FechaSQL(ffin) & "'  and " & _
                                              "isnull(cod_super, '') <> '' and COD_PLANTA = '" & IIf(sbPlanta.Value, sbPlanta.ValueTrue, sbPlanta.ValueFalse) & "' " & _
                                              "and alta <= '" & FechaSQL(ffin) & "' and (baja is null or baja >= '" & FechaSQL(ffin) & "')  order by RELOJ asc", "TA")
                End If
                ...
                If ffin >= Date.Now Then
                    ...
                    chkConfirmarOH.Enabled = False                'MODIFICADO     18/ENE/20
        ...
        Try
            ...
            If dtPeriodo.Rows.Count > 0 Then
                ...
                For Each row As DataRow In dtMiPersonal.Select(filtroGeneral, "reloj")
                    ...
                    chkConfirmarOH.Checked = OH_confirmado
                    chkConfirmarOH.Visible = False                'MODIFICADO     18/ENE/20
            ...
            If h.Contains("AUT") Then
                ...
                Else
                    Try
                        ...
                        'horas = Double.Parse(_valor)                            'MODIFICADO     18/ENE/20
                        If horas >= 0 Then
                            'MCR OCT-26-2020
                        ...
                        For Each cl As DataGridViewColumn In dgvExtraAut.Columns
                            If cl.Name.Contains("AUT") Then
                                ...
                                If _autorizado IsNot Nothing Then
                                    ...
                                    Dim horas As Double = -1
                                    horas = Double.Parse(_autorizado)
                                    If horas >= 0 Then
                                        'MCR OCT-26-2020
                                        'Solo permitir horas completas
        ...
        Try
            ...
            If autorizar_global Then
                ...
                For Each drow As DataGridViewRow In dgvExtraAut.Rows
                    ...
                    For Each cl As DataGridViewColumn In dgvExtraAut.Columns
                        If cl.Name.Contains("AUT") Then
                            Dim _autorizado As String = drow.Cells(cl.Name).Value
                            If _autorizado IsNot Nothing Then
                                _autorizado = _autorizado.Replace(":", ".")
                                Dim horas As Double = -1
                                'horas = Double.Parse(_autorizado)                       'MODIFICADO     18/ENE/20
                                If horas >= 0 Then
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        Try
            Timer1.Stop()
            Dim dtPeriodoActivo As DataTable = Nothing
      
            dtPeriodoActivo = sqlExecute("select ano+periodo as anoper from periodos where activo = 1 and  isnull(periodo_especial, 0) = 0 order by ano+periodo desc", "TA")
    ...
    Private Sub chkConfirmarOH_CheckedChanged(sender As Object, e As EventArgs) 'Handles chkConfirmarOH.CheckedChanged
        Try
            ...
            If chkConfirmarOH.Checked = True Then
                If MessageBox.Show("¿Desea marcar como confirmados todos los registros de los empleados en la lista para la compañia OutHelping?", "Confirmar todo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                    ...
                    For Each dgrv As DataGridViewRow In dgvNomSem.Rows
                        Application.DoEvents()
                        If dgrv.Cells("colComp").Value = "002" Then
                ...
                Else
                    ...
                    chkConfirmarOH.Checked = False
                End If
            ElseIf chkConfirmarOH.Checked = False Then
                If MessageBox.Show("¿Desea marcar como no confirmados todos los registros de los empleados en la lista para la compañia OutHelping?", "Confirmar todo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
