Public Class frmFiltroAUS
    ...
    Private Sub frmFiltro_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dtCampos = sqlExecute("SELECT RTRIM(UPPER(nombre)) AS 'NOMBRE',UPPER(cod_campo) as 'COD_CAMPO', tipo, tabla FROM TA.dbo.Campos_AUS ", "TA")
        ...
        Try
            ...
            Select Case Tipo
                ...
                Case "char"
                    ...
                    dtIncluyendo = sqlExecute("SELECT DISTINCT TOP 100 " & Campo & "  FROM " & Tabla, "TA")
    ...
    Private Sub ListarResultado(valor As String)
        Dim dtResultadoPers As New DataTable
        Try
            If valor.Length > 0 Then
                dtResultadoPers = dtResultadoAUS.Clone
                For Each dRow In dtResultadoAUS.Select(valor)
                    dtResultadoPers.ImportRow(dRow)
                Next
            Else
                dtResultadoPers = dtResultadoAUS.Copy
            End If

            dgLista.DataSource = dtResultadoPers
        Catch ex As Exception
            ErrorLog(Usuario, System.Reflection.MethodBase.GetCurrentMethod.Name(), Me.Name, ex.HResult, ex.Message)
        End Try
    ...
    Private Sub frmFiltroAUS_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
        e.Cancel = True
    End Sub
End Class