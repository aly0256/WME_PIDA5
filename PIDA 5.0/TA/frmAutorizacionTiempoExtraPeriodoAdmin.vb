Public Class frmAutorizacionTiempoExtraPeriodoAdmin
    ...
    Public Sub carga_estructura()
        ...
        Try
            ...
            If dtPeriodo.Rows.Count > 0 Then
                ...
                dtSupervisores.Columns.Add("adminn")
                ...
                dtMiPersonal = ConsultaPersonalVW("select reloj, nombres, cod_planta, cod_turno, cod_hora, cod_depto, cod_clase, isnull(cod_super, '') as cod_super, isnull(nombre_super, 'Sin Supervisor') as nombre_super, 'Administrativos' as Adminn from personalvw where cod_tipo = 'A' and alta <= '" & FechaSQL(ffin) & "' and (baja is null or baja >= '" & FechaSQL(fini) & "')  and cod_comp in ('610') and isnull(cod_super, '') <> '' and cod_planta = '" & IIf(sbPlanta.Value, sbPlanta.ValueTrue, sbPlanta.ValueFalse) & "' order by reloj", False)
                ...
                For Each row As DataRow In dtMiPersonal.Rows
                    ...
                    Try
                        Dim drow As DataRow = dtSupervisores.Rows.Find(row("adminn"))
                        If drow Is Nothing Then
                            dtSupervisores.Rows.Add(row("Adminn"), row("adminn"), 1)
                        Else
                ...
                n_s.Text = "Administrativos"
                n_s.Tag = "TODOS"

                For Each row As DataRow In dtSupervisores.Select("", "adminn")
                    Dim n As New DevComponents.AdvTree.Node
                    n.Text = RTrim(row("adminn")) & Space(1) & "- " & row("cuantos") & ""
                    n.Tag = row("adminn")
    ...
    Public Sub cargar_informacion(anoper As String, cod_super As String)
        Try
            ...
            If dtPeriodo.Rows.Count > 0 Then
                ...
                Dim filtro As String = IIf(cod_super = "TODOS", "", "cod_super = '" & cod_super & "'")
    ...
    Private Sub SuperTabControl1_SelectedTabChanged(sender As Object, e As DevComponents.DotNetBar.SuperTabStripSelectedTabChangedEventArgs) Handles tabDatos.SelectedTabChanged
        ...
        Else
            ...
            btnReporteAutorizacion.Visible = True
    ...
    Private Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles txtHorasAutorizacion.Validating
        Try
            Dim horas As Double = -1
            horas = Double.Parse(txtHorasAutorizacion.Text)
            If horas >= 0 Then
                Dim valor As String = txtHorasAutorizacion.Text
                txtHorasAutorizacion.Text = MerMilitar(CtoH(txtHorasAutorizacion.Text))
    ...
    Public Sub revisar_colores(fini As Date, ffin As Date, Optional fila As Integer = -1)
    ...
    Private Sub picAyuda_Click(sender As Object, e As EventArgs) Handles picAyuda.Click
        Try
    ...
    Private Sub chkConfirmarTodo_CheckedChanged(sender As Object, e As EventArgs) ' Handles chkConfirmarTodo.CheckedChanged
        ...
        Try
            ...
            If chkConfirmarTodo.Checked = True Then
                If MessageBox.Show("¿Desea marcar como confirmados todos los registros de los empleados en la lista?", "Confirmar todo", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub btnReporteAvance_Click(sender As Object, e As EventArgs) Handles btnReporteAvance.Click
        Try
            ...
            If dtPeriodo.Rows.Count > 0 Then
                ...
                Dim dtPersonalReporte As DataTable = ConsultaPersonalVW("select reloj, nombres, cod_planta, cod_turno, cod_depto, cod_clase,isnull(cod_clerk, '') as cod_clerk, isnull(nombre_clerk, '') as nombre_clerk, isnull(cod_super, '') as cod_super, isnull(nombre_super, 'Sin Supervisor') as nombre_super from personalvw where cod_tipo = 'A' and alta <= '" & FechaSQL(ffin) & "' and (baja is null or baja >= '" & FechaSQL(fini) & "') and cod_comp in ('610') and isnull(cod_super, '') <> '' and  cod_planta = '" & IIf(sbPlanta.Value, sbPlanta.ValueTrue, sbPlanta.ValueFalse) & "' order by cod_clerk,reloj", False)
                ...
                dtAvanceSupervisores.Columns.Add("cod_clerk")
                dtAvanceSupervisores.Columns.Add("nombre_clerk")
                ...
                For Each row As DataRow In dtPersonalReporte.Rows
                    ...
                    Dim drow As DataRow = dtAvanceSupervisores.Rows.Find({row("cod_planta"), row("cod_super")})
                ...
                Dim dSuper As DataTable = sqlExecute("select distinct cod_super, nombre, cod_clerk from super where cod_comp in ('610') order by cod_clerk")
    ...
    Private Sub btnReporteAutorizacion_Click(sender As Object, e As EventArgs) Handles btnReporteAutorizacion.Click
        Try
            ...
            Else
                Dim filtro As String = "cod_super = '" & cod_super & "'"
                Dim filtro_text As String = RTrim(advSupervisores.SelectedNode.Text.Split("-")(0)) & ", " & IIf(sbPlanta.Value = False, "Juárez 1", "Juárez 2")
                ...
                For Each row As DataRow In dtMiPersonal.Select(filtro, "reloj")
                    ...
                    Dim dtNomsemEmpleado As DataTable = sqlExecute("select nomsem.*, personalvw.cod_turno, personalvw.alta, personalvw.cod_tipo, case when fecha_firma is null then 0 else 1 end as confirm, rtrim(isnull(usuario_firma, '')) as usuario_auth, personalvw.nombres from nomsem left join personal.dbo.personalvw on personalvw.reloj = nomsem.reloj where nomsem.reloj = '" & row("reloj") & "' and nomsem.ano+nomsem.periodo = '" & anoper & "'", "TA")
