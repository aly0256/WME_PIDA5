Public Class frmFiltroTAlloc
    ...
    Private Sub frmFiltro_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dtCampos = sqlExecute("SELECT RTRIM(UPPER(nombre)) AS 'NOMBRE',UPPER(cod_campo) as 'COD_CAMPO',tipo,ISNULL(tabla,'TimeAllocationVW') as tabla " & _
                              "FROM TA.dbo.camposTAlloc ", "TA")
    ...
    Private Function FiltrarDatos(Texto As String, Excepcion As Boolean) As DataTable
        ...
        Else
            Campo = ""
        End If
        dtValores = sqlExecute("SELECT DISTINCT " & Campo & "  FROM timeallocationvw WHERE " & FiltraValor(Campo, Texto, Excepcion), "ta")
        Return dtValores
    End Function
    ...
    Private Sub ListarResultado(valor As String)
        ...
        Try
            ...
            dtResultado = dtResultadoTAlloc.Clone
            For Each dRow In dtResultadoTAlloc.Select(valor)
