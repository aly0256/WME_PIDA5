Public Class frmEditarAjPro_esp
    ...
    Private Sub frmEditarAjPro_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Reloj = frmIndivNomEsp.txtReloj.Text.Trim
            anio = frmIndivNomEsp.lblAnioPer.Text.Trim.Substring(0, 4)
            per = frmIndivNomEsp.lblAnioPer.Text.Trim.Substring(7, 2)
            Dim QConcAju As String = ""
            If (nombreNomEspecial = "agui") Then QConcAju = "select * from conc_ajustes_pro where concepto in('PENALI','PENAL2','PENAL3','OTRASD','OTPGRA','PERAGI')"
            If (nombreNomEspecial = "liq_fah") Then QConcAju = "select * from conc_ajustes_pro where concepto in('PENALI','PENAL2','PENAL3','OTRASD','OTPGRA','LIFAHC','LIFAHE','LIINTF','ABOPMO')"
            If (nombreNomEspecial = "ptu") Then QConcAju = "select * from conc_ajustes_pro where concepto in('PERPTU','PTUDIA','PTUSUE')"


            '--Cargar conceptos, en este caso solo las de aguinaldo
            dtConcAjustes = sqlExecute(QConcAju, "NOMINA")
            ...
            dtAjustesPro = sqlExecute("SELECT * from ajustes_pro_esp where reloj='" & Reloj & "' and concepto='" & CalcIndivKey & "'", "NOMINA")
    ...
    Private Sub btnAceptar_Click(sender As Object, e As EventArgs) Handles btnAceptar.Click
        ...
        Try
            ...
            If (Nuevo) Then ' Nuevo registro
                Dim dtExiste As DataTable = sqlExecute("select * from ajustes_pro_esp where reloj='" & Reloj & "' and concepto='" & cmbConcep.SelectedValue & "'", "Nomina")
                ...
                Else
                    ...
                    Dim QI As String = "insert into ajustes_pro_esp (ano,periodo,reloj,per_ded,concepto,descrip,monto,origen,saldo,comentario) " & _
            ...
            Else '-- Editar registro
                ...
                Dim QUp As String = "update ajustes_pro_esp set monto=" & txtMonto.Value & ",origen='" & Usuario & "',comentario='" & txtComentario.Text & "' where reloj='" & Reloj & "' and concepto='" & concepto & "'"
