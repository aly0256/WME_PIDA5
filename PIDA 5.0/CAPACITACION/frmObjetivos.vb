...
#End Region
    ...
    Private Sub frmObjetivos_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            dtLista = sqlExecute("SELECT objetivo as 'Código',Nombre FROM objetivos", "capacitacion")
            dtLista.DefaultView.Sort = "Código"
            dgTabla.DataSource = dtLista
            dgTabla.Columns(1).AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill

            dtRegistro = sqlExecute("SELECT TOP 1 * FROM objetivos ORDER BY objetivo ASC ", "capacitacion")
            MostrarInformacion()
    ...
    Private Sub MostrarInformacion()
        ...
        Try
            ...
            If dtRegistro.Rows.Count > 0 Then
                ...
                txtCodigo.Text = dtRegistro.Rows(0).Item("objetivo")
        ...
        Catch ex As Exception
            MessageBox.Show("Se presentó un error al intentar mostrar la información del objetivo.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        Dim Cod As String
        Cod = Buscar("capacitacion.dbo.objetivos", "objetivo", "objetivos", False)
        If Cod <> "CANCELAR" Then
            dtRegistro = sqlExecute("SELECT * FROM objetivos WHERE objetivo = '" & Cod & "' ", "Capacitacion")
            MostrarInformacion()
        End If
    End Sub

    Private Sub btnPrimero_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrimero.Click
        Primero("objetivos", "objetivo", dtRegistro, "capacitacion")
        MostrarInformacion()
    End Sub

    Private Sub btnCiasPrev_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAnterior.Click
        Anterior("objetivos", "objetivo", txtCodigo.Text, dtRegistro, "capacitacion")
        MostrarInformacion()
    End Sub

    Private Sub btnSiguiente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSiguiente.Click
        Siguiente("objetivos", "objetivo", txtCodigo.Text, dtRegistro, "capacitacion")
        MostrarInformacion()
    End Sub

    Private Sub btnBorrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBorrar.Click
        Codigo = txtCodigo.Text
        dtTemporal = sqlExecute("SELECT cod_curso FROM cursos WHERE objetivo = '" & Codigo & "'", "capacitacion")
        ...
        Else
            If MessageBox.Show("¿Está seguro de borrar el registro " & Codigo & "?", "Borrar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                sqlExecute("DELETE FROM objetivos WHERE objetivo = '" & Codigo & "'", "capacitacion")
    ...
    Private Sub dgTabla_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgTabla.RowEnter
        ...
        dtRegistro = sqlExecute("SELECT * FROM objetivos WHERE objetivo = '" & cod & "' AND nombre = '" & nom & "'", "capacitacion")
        MostrarInformacion()
    End Sub
    ...
    Private Sub btnUltimo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUltimo.Click
        Ultimo("objetivos", "objetivo", dtRegistro, "capacitacion")
        MostrarInformacion()
    End Sub

    Private Sub btnNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNuevo.Click
        If Agregar Then
            ' Si Agregar, revisar si existe objetivo
            dtTemporal = sqlExecute("SELECT objetivo FROM objetivos where objetivo = '" & txtCodigo.Text & "'", "capacitacion")
            If dtTemporal.Rows.Count > 0 Then
                MessageBox.Show("El registro no se puede agregar, ya existe '" & txtCodigo.Text & "'.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                txtCodigo.Focus()
                Exit Sub
            Else
                sqlExecute("INSERT INTO objetivos (objetivo,nombre) VALUES ('" & txtCodigo.Text & "','" & txtNombre.Text & "')", "capacitacion")
                Agregar = False
            End If

        ElseIf Editar Then
            ' Si Editar, entonces guardar cambios a registro
            sqlExecute("UPDATE objetivos SET nombre = '" & txtNombre.Text & "' WHERE objetivo = '" & txtCodigo.Text & "'", "capacitacion")
        Else
            Agregar = True
        End If
        Editar = False
    ...
    Private Sub btnReporte_Click(sender As Object, e As EventArgs) Handles btnReporte.Click
        frmVistaPrevia.LlamarReporte("objetivos", Nothing)
