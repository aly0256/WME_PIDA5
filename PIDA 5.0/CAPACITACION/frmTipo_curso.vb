...
#End Region
    ...
    Private Sub frmTipo_curso_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            dtLista = sqlExecute("SELECT cod_tipo_curso as 'Código',Nombre FROM Tipo_curso", "capacitacion")
            dtLista.DefaultView.Sort = "Código"
            dgTabla.DataSource = dtLista
            dgTabla.Columns(1).AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill

            dtRegistro = sqlExecute("SELECT TOP 1 * FROM Tipo_curso ORDER BY cod_tipo_curso ASC ", "capacitacion")
            MostrarInformacion()
    ...
    Private Sub MostrarInformacion()
        ...
        Try
            ...
            If dtRegistro.Rows.Count > 0 Then
                txtCodigo.Text = dtRegistro.Rows(0).Item("cod_tipo_curso")
        ...
        Catch ex As Exception
            MessageBox.Show("Se presentó un error al intentar mostrar la información del tipo curso", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        Dim Cod As String
        Cod = Buscar("capacitacion.dbo.Tipo_curso", "cod_tipo_curso", "Tipo_curso", False)
        If Cod <> "CANCELAR" Then
            dtRegistro = sqlExecute("SELECT * FROM Tipo_curso WHERE cod_tipo_curso = '" & Cod & "' ", "Capacitacion")
            MostrarInformacion()
        End If
    End Sub

    Private Sub btnPrimero_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrimero.Click
        Primero("Tipo_curso", "cod_tipo_curso", dtRegistro, "capacitacion")
        MostrarInformacion()
    End Sub

    Private Sub btnCiasPrev_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAnterior.Click
        Anterior("Tipo_curso", "cod_tipo_curso", txtCodigo.Text, dtRegistro, "capacitacion")
        MostrarInformacion()
    End Sub

    Private Sub btnSiguiente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSiguiente.Click
        Siguiente("Tipo_curso", "cod_tipo_curso", txtCodigo.Text, dtRegistro, "capacitacion")
        MostrarInformacion()
    End Sub

    Private Sub btnBorrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBorrar.Click
        Codigo = txtCodigo.Text
        dtTemporal = sqlExecute("SELECT cod_curso FROM cursos WHERE cod_tipo_curso = '" & Codigo & "'", "capacitacion")
        ...
        Else
            If MessageBox.Show("¿Está seguro de borrar el registro " & Codigo & "?", "Borrar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                sqlExecute("DELETE FROM Tipo_curso WHERE cod_tipo_curso = '" & Codigo & "'", "capacitacion")
    ...
    Private Sub dgTabla_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgTabla.RowEnter
        ...
        dtRegistro = sqlExecute("SELECT * FROM Tipo_curso WHERE cod_tipo_curso = '" & cod & "' AND nombre = '" & nom & "'", "capacitacion")
        MostrarInformacion()
    End Sub
    ...
    Private Sub btnUltimo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUltimo.Click
        Ultimo("Tipo_curso", "cod_tipo_curso", dtRegistro, "capacitacion")
        MostrarInformacion()
    End Sub

    Private Sub btnNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNuevo.Click
        If Agregar Then
            ' Si Agregar, revisar si existe cod_tipo_curso
            dtTemporal = sqlExecute("SELECT cod_tipo_curso FROM Tipo_curso where cod_tipo_curso = '" & txtCodigo.Text & "'", "capacitacion")
            If dtTemporal.Rows.Count > 0 Then
                MessageBox.Show("El registro no se puede agregar, ya existe '" & txtCodigo.Text & "'.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                txtCodigo.Focus()
                Exit Sub
            Else
                sqlExecute("INSERT INTO Tipo_curso (cod_tipo_curso,nombre) VALUES ('" & txtCodigo.Text & "','" & txtNombre.Text & "')", "capacitacion")
                Agregar = False
            End If

        ElseIf Editar Then
            ' Si Editar, entonces guardar cambios a registro
            sqlExecute("UPDATE Tipo_curso SET nombre = '" & txtNombre.Text & "' WHERE cod_tipo_curso = '" & txtCodigo.Text & "'", "capacitacion")
        Else
            Agregar = True
        End If
        Editar = False
    ...
    Private Sub btnReporte_Click(sender As Object, e As EventArgs) Handles btnReporte.Click
        frmVistaPrevia.LlamarReporte("Tipo_curso", Nothing)
