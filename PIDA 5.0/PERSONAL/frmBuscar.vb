Public Class frmBuscar
    ...
    Private Sub btnAceptar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAceptar.Click
        Try
            If txtReloj.Text <> "" Then
                Reloj = txtReloj.Text
                Reloj2 = txtReloj.Text
            End If
            'CAMPO DE RECIBOS DE EMPLEADO       24/NOV/2020
            If frmRecibos.varActivoRecibos Then
                frmRecibos.txtReloj.Text = txtReloj.Text
            End If
    ...
    Private Sub frmBuscar_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            '== Como en algunas ventanas el perfil de gerente medico puede 
            '== visualizar a todos los empleados pero en otras no, condicion de acuerdo a su perfil                 Ernesto         2mayo2022
            If Perfil.Contains("GERENTE_MEDICO") And formaAbierta Then
                dtBuscar = FiltraInformacion(ConsultaPersonalVW("SELECT RELOJ,NOMBRES AS NOMBRE,ALTA,BAJA,RTRIM(COD_DEPTO) + ' - ' + RTRIM(NOMBRE_DEPTO) AS DEPARTAMENTO," & _
                                  "RTRIM(COD_PUESTO) + ' - ' + RTRIM(NOMBRE_PUESTO) AS PUESTO,PERSONALVW.COD_COMP,FOTO,'' AS CAMPOS_EXTRA" & _
                                  IIf(CamposBusca.Length > 0, "," & CamposBusca, "") & " FROM PERSONALVW " & _
                                  "WHERE (nivel_seguridad IS NULL OR nivel_seguridad <=" & NivelConsulta & ")", False), "cod_super")
            End If
    ...
    Private Sub txtBusca_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtBusca.TextChanged
        ...
        Try
            ...
            If i >= 0 Then
                ...
                vl = "(reloj LIKE '%" & vlAnt & "%' OR nombre LIKE '%" & vlAnt & "%' OR campos_extra LIKE '%" & vlAnt & "%') AND (" & _
                    "reloj LIKE '%" & vlDes & "%' OR nombre LIKE '%" & vlDes & "%' OR campos_extra LIKE '%" & vlDes & "%')"
            Else
                vl = "reloj LIKE '%" & vl & "%' OR nombre LIKE '%" & vl & "%' OR campos_extra LIKE '%" & vl & "%'"
            End If
    ...
    Private Sub dgTabla_CellContentDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgTabla.CellContentDoubleClick
        Reloj = dgTabla.Item("colreloj", dgTabla.CurrentRow.Index).Value
