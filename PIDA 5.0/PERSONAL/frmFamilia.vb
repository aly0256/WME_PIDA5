...
#End Region
    ...
    Private Sub frmcod_familia_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            dtLista = sqlExecute("SELECT cod_familia as 'Código',Nombre FROM familia")
            dtLista.DefaultView.Sort = "Código"
            dgTabla.DataSource = dtLista
            dgTabla.Columns(1).AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill

            dtRegistro = sqlExecute("SELECT TOP 1 * FROM familia ORDER BY cod_familia ASC")
            MostrarInformacion()
    ...
    Private Sub MostrarInformacion()
        ...
        txtCodigo.Text = dtRegistro.Rows(0).Item("cod_familia")
        txtNombre.Text = dtRegistro.Rows(0).Item("nombre").ToString.Trim
        btnSexo.Value = IIf(dtRegistro.Rows(0).Item("sexo").ToString.Trim.Equals("F"), True, False)
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        Dim Cod As String
        Cod = Buscar("familia", "cod_familia", "familia", False)
        If Cod <> "CANCELAR" Then
            dtRegistro = sqlExecute("SELECT * FROM familia WHERE cod_familia = '" & Cod & "' ")
            MostrarInformacion()
        End If
    End Sub

    Private Sub btnPrimero_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrimero.Click
        Primero("familia", "cod_familia", dtRegistro)
        MostrarInformacion()
    End Sub

    Private Sub btnCiasPrev_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAnterior.Click
        Anterior("familia", "cod_familia", txtCodigo.Text, dtRegistro)
        MostrarInformacion()
    End Sub

    Private Sub btnSiguiente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSiguiente.Click
        Siguiente("familia", "cod_familia", txtCodigo.Text, dtRegistro)
        MostrarInformacion()
    End Sub

    Private Sub btnBorrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBorrar.Click
        Codigo = txtCodigo.Text
        dtTemporal = sqlExecute("SELECT reloj from personalvw WHERE cod_familia = '" & Codigo & "'")
        ...
        Else
            If MessageBox.Show("¿Está seguro de borrar el registro " & Codigo & "?", "Borrar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                sqlExecute("DELETE FROM familia WHERE cod_familia = '" & Codigo & "'")
    ...
    Private Sub dgTabla_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgTabla.RowEnter
        ...
        dtRegistro = sqlExecute("SELECT * FROM familia WHERE cod_familia = '" & familiar & "'")
        MostrarInformacion()
    End Sub
    ...
    Private Sub btnUltimo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUltimo.Click
        Ultimo("familia", "cod_familia", dtRegistro)
        MostrarInformacion()
    End Sub
    ...
    Private Sub btnNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNuevo.Click
        If Agregar Then
            ...
            dtTemporal = sqlExecute("SELECT cod_familia FROM familia where cod_familia = '" & txtCodigo.Text & "'")
            If dtTemporal.Rows.Count > 0 Then
                MessageBox.Show("El registro no se puede agregar, ya existe el tipo de familiar '" & txtCodigo.Text & "'.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                txtCodigo.Focus()
                Exit Sub
            Else
                sqlExecute("INSERT INTO familia (cod_familia, nombre, sexo) VALUES ('" & txtCodigo.Text & "','" & txtNombre.Text & "', '" & IIf(btnSexo.Value, "F", "M") & "')")
                Agregar = False
            End If

        ElseIf Editar Then
            ' Si Editar, entonces guardar cambios a registro
            sqlExecute("UPDATE familia SET nombre = '" & txtNombre.Text & "', sexo = '" & IIf(btnSexo.Value, "F", "M") & "' WHERE cod_familia = '" & txtCodigo.Text & "'")
        Else
            Agregar = True
        End If
        Editar = False
    ...
    Private Sub btnReporte_Click(sender As Object, e As EventArgs) Handles btnReporte.Click
        frmVistaPrevia.LlamarReporte("Familia", Nothing)
