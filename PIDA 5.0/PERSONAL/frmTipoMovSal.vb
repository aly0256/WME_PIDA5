...
#End Region
    ...
    Private Sub frmcod_tipo_mod_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            dtLista = sqlExecute("SELECT cod_tipo_mod as 'Código',Nombre FROM tipo_mod_sal")
            dtLista.DefaultView.Sort = "Código"
            dgTabla.DataSource = dtLista
            dgTabla.Columns(1).AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill

            dtRegistro = sqlExecute("SELECT TOP 1 * FROM tipo_mod_sal ORDER BY cod_tipo_mod ASC")
            MostrarInformacion()
    ...
    Private Sub MostrarInformacion()
        ...
        txtCodigo.Text = dtRegistro.Rows(0).Item("cod_tipo_mod")
    ...
    Private Sub btnBuscar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBuscar.Click
        Dim Cod As String
        Cod = Buscar("tipo_mod_sal", "cod_tipo_mod", "tipo_mod_sal", False)
        If Cod <> "CANCELAR" Then
            dtRegistro = sqlExecute("SELECT * FROM tipo_mod_sal WHERE cod_tipo_mod = '" & Cod & "' ")
            MostrarInformacion()
        End If
    End Sub

    Private Sub btnPrimero_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPrimero.Click
        Primero("tipo_mod_sal", "cod_tipo_mod", dtRegistro)
        MostrarInformacion()
    End Sub

    Private Sub btnCiasPrev_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAnterior.Click
        Anterior("tipo_mod_sal", "cod_tipo_mod", txtCodigo.Text, dtRegistro)
        MostrarInformacion()
    End Sub

    Private Sub btnSiguiente_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSiguiente.Click
        Siguiente("tipo_mod_sal", "cod_tipo_mod", txtCodigo.Text, dtRegistro)
        MostrarInformacion()
    End Sub

    Private Sub btnBorrar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBorrar.Click
        Codigo = txtCodigo.Text
        dtTemporal = sqlExecute("SELECT reloj from personalvw WHERE cod_tipo_mod = '" & Codigo & "'")
        ...
        Else
            If MessageBox.Show("¿Está seguro de borrar el registro " & Codigo & "?", "Borrar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                sqlExecute("DELETE FROM tipo_mod_sal WHERE cod_tipo_mod = '" & Codigo & "'")
    ...
    Private Sub dgTabla_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgTabla.RowEnter
        ...
        Dim mov As String
        mov = dgTabla.Item("Código", e.RowIndex).Value
        dtRegistro = sqlExecute("SELECT * FROM tipo_mod_sal WHERE cod_tipo_mod = '" & mov & "' ")
        MostrarInformacion()
    End Sub
    ...
    Private Sub btnUltimo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnUltimo.Click
        Ultimo("tipo_mod_sal", "cod_tipo_mod", dtRegistro)
        MostrarInformacion()
    End Sub

    Private Sub btnNuevo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnNuevo.Click
        If Agregar Then
            ' Si Agregar, revisar si existe cod_tipo_mod
            dtTemporal = sqlExecute("SELECT cod_tipo_mod FROM tipo_mod_sal where cod_tipo_mod = '" & txtCodigo.Text & "'")
            If dtTemporal.Rows.Count > 0 Then
                MessageBox.Show("El registro no se puede agregar, ya existe el tipo de tipo_mod_salr '" & txtCodigo.Text & "'.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                txtCodigo.Focus()
                Exit Sub
            Else
                sqlExecute("INSERT INTO cod_tipo_mod (cod_tipo_mod,nombre) VALUES ('" & txtCodigo.Text & "','" & txtNombre.Text & "')")
                Agregar = False
            End If

        ElseIf Editar Then
            ' Si Editar, entonces guardar cambios a registro
            sqlExecute("UPDATE cod_tipo_mod SET nombre = '" & txtNombre.Text & "' WHERE cod_tipo_mod = '")
        Else
            Agregar = True
        End If
        Editar = False
    ...
    Private Sub btnReporte_Click(sender As Object, e As EventArgs) Handles btnReporte.Click
        frmVistaPrevia.LlamarReporte("Movimientos de sueldo", Nothing)
