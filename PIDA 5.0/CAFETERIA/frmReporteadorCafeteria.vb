...
Imports System.Xml
Public Class frmReporteadorCafeteria
    ...
    Private Sub frmReporteadorTA_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        ...
        Me.Dispose()
    End Sub
    Private Sub frmReporteadorCAfeteria_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ActualizarCostes()
        ...
        Try
            ...
            If chkRecientes.Checked Then
                Cadena = "SELECT RTRIM(reportes.NOMBRE) AS NOMBRE,reportes.TIPO," & _
                    ...
                    " cod_perfil " & Perfil & " AND permisos.modulo = 'CAF'  and permisos.acceso = 1)" & _
            ...
            End If
            dtReportes = New DataTable
            dtReportes = sqlExecute(Cadena, "Cafeteria")
    ...
    Private Sub Mientras()
        ...
        Try
            ...
            dtTemporal = sqlExecute("UPDATE reportes SET veces_acceso = veces_acceso + 1, fecha = GETDATE()   WHERE nombre = '" & NombreReporte.Trim & "'", "Cafeteria")

            dtTemporal = sqlExecute("SELECT COUNT(nombre) AS cuantos FROM reportes_recientes WHERE usuario = '" & Usuario & "'", "Cafeteria")
            If dtTemporal.Rows.Item(0).Item("cuantos") = 10 Then
                dtTemporal = sqlExecute("DELETE FROM reportes_recientes WHERE fecha = (SELECT MIN(fecha) FROM reportes_recientes WHERE usuario = '" & Usuario & "') AND usuario = '" & Usuario & "'", "Cafeteria")
            End If

            dtTemporal = sqlExecute("SELECT nombre FROM reportes_recientes WHERE usuario = '" & Usuario & "' AND nombre = '" & NombreReporte.Trim & "'", "Cafeteria")
            If dtTemporal.Rows.Count > 0 Then
                dtTemporal = sqlExecute("UPDATE reportes_recientes SET fecha = GETDATE() WHERE usuario = '" & Usuario & "' AND nombre = '" & NombreReporte.Trim & "'", "Cafeteria")
            Else
                dtTemporal = sqlExecute("INSERT INTO reportes_recientes (usuario,fecha,nombre) VALUES ('" & Usuario & "',GETDATE(), '" & NombreReporte & "')", "Cafeteria")
            End If

            FiltroReporte = FiltrosAcumulados()
            OrdenReporte = OrdenAcumulado()

            Dim dtResultadoCafeteriaFiltro As New DataSet
            dtResultadoCafeteriaFiltro.Merge(dtResultadoCafeteria.Select(FiltroReporte))

            If dtResultadoCafeteriaFiltro.Tables.Count = 0 Then
                dtFiltroTA = New DataTable
            Else
                dtFiltroTA = dtResultadoCafeteriaFiltro.Tables(0)
                dtResultadoCafeteriaFiltro.Tables(0).TableName = "Cafeteria"
                dtFiltroTA.DefaultView.Sort = OrdenReporte
            End If

            If Tipo = "U" Then
                dtDisponibles = sqlExecute("SELECT UPPER(cod_campo) AS campo,upper(left(nombre,1)) + lower(substring(nombre,2,75)) AS nombre,tipo FROM campos ORDER BY nombre", "Cafeteria")
                ReporteadorFuente = "TAA"
                dtReporte = sqlExecute("SELECT * FROM reportes WHERE nombre = '" & NombreReporte.Trim & "'", "Cafeteria")
...
ErrS:
    ...
    Private Sub btnNuevo_Click(sender As Object, e As EventArgs) Handles btnNuevo.Click
        Try
            ...
            dtCampos = sqlExecute("select upper(cod_campo) as campo,nombre,tipo FROM campos", "Cafeteria")
                ...
                For x = 0 To UBound(Campos)
                    ...
                    If Not IsNothing(dr) Then
                        If dr("tipo").ToString.Trim = "num" Then
                            Valor = Format(IIf(IsDBNull(dRow(Campos(x))), 0, dRow(Campos(x))), "C")
                        Else
    ...
    Private Sub btnBorrar_Click(sender As Object, e As EventArgs) Handles btnBorrar.Click
        Try
            ...
            If MessageBox.Show("¿Está seguro de eliminar el reporte " & NombreReporte & "?", "Borrar reporte", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
                dtTemporal = sqlExecute("DELETE FROM reportes WHERE nombre = '" & NombreReporte.Trim & "'", "Cafeteria")
    ...
    Private Sub bgwReporte_DoWork()
        ...
        Try
            ...
            dtFiltroPersonal = dtResultadoCafeteria.Clone
            For Each dR As DataRow In dtResultadoCafeteria.Select(FiltroReporte, OrdenReporte)
                dtFiltroPersonal.ImportRow(dR)
            Next
            '    dtResultadoCafeteria = dtResultadoCafeteriatemp.Copy
            ...
            If Tipo = "U" Then
                dtDisponibles = sqlExecute("SELECT UPPER(cod_campo) AS campo,personal.dbo.IniciaMayuscula(nombre) AS nombre,tipo " & _
                                           "FROM campos ORDER BY nombre", "Cafeteria")
                ReporteadorFuente = "TAA"
                dtReporte = sqlExecute("SELECT * FROM reportes WHERE nombre = '" & NombreReporte & "'", "Cafeteria")
                If dtReporte.Rows.Count < 1 Then Exit Sub
                frmVistaPrevia.ReporteDinamico(dtReporte.Rows(0), dtFiltroPersonal, NombreReporte)
            ElseIf NombreReporte.ToUpper = "VALE DE COMIDA" Then
                Dim dtplanta As New DataTable
                Dim planta As String
                Dim suc As String
                dtplanta = sqlExecute("select path_reportes from parametros")
                planta = IIf(IsDBNull(dtplanta.Rows(0).Item("path_reportes")), "", dtplanta.Rows(0).Item("path_reportes"))
                If planta.Contains("MXJU") Then
                    suc = "PLANTA 1"
                ElseIf planta.Contains("MXJZ") Then
                    suc = "PLANTA 2"
                Else
                    suc = "N/A"
                End If
                dtplanta.Columns.Add("suc")

                For Each dr As DataRow In dtplanta.Rows
                    dr("suc") = suc
                Next
                frmVistaPrevia.LlamarReporte(NombreReporte.Trim, dtplanta, CiaReporteador.Replace("''", ""))
    ...
    Private Sub bgw_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bgw.DoWork
        Dim dtPersonalFaltante As New DataTable
        Try
            dtResultadoCafeteria = ObtenerPersonalCafeteria()
            'For Each dEmp As DataRow In dtPersonalFaltante.Rows
            '    dtResultadoCafeteria.ImportRow(dEmp)
            'Next
        Catch ex As Exception
            ErrorLog(Usuario, System.Reflection.MethodBase.GetCurrentMethod.Name(), Me.Name, ex.HResult, ex.Message)
        End Try
    ...
    Private Sub bgw_RunWorkerCompleted(sender As Object, e As System.ComponentModel.RunWorkerCompletedEventArgs) Handles bgw.RunWorkerCompleted
        ...
        cmbTipoReportes.DataSource = sqlExecute("SELECT * FROM tipo_reportes ORDER BY tipo", "Cafeteria")
    ...
    End Sub
    Private Function ObtenerPersonalCafeteria(Optional FiltroUsuarioCafeteria As String = "") As DataTable
        Dim dtTemp As DataTable = New DataTable
        Dim query As String = " select * from marcajesVW where cast(fechahora as date) between '" + FechaHoraSQL(FechaInicial) + "' and '" + FechaHoraSQL(FechaFinal) + "'"
        dtTemp = sqlExecute(query, "Cafeteria")
        Return dtTemp
    End Function
