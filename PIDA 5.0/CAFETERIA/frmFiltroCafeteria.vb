Public Class frmFiltroCafeteria
    ...
    Private Sub frmFiltro_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dtCampos = sqlExecute("SELECT RTRIM(UPPER(nombre)) AS 'NOMBRE',UPPER(cod_campo) as 'COD_CAMPO',tipo,ISNULL(tabla,'marcajesVW') as tabla," & _
                              "ISNULL(campo_en_tabla,cod_campo) as campo_en_tabla FROM campos ", "Cafeteria")
        ...
        Try
            ...
            Select Case Tipo
                ...
                Case "char"
                    ...
                    dtIncluyendo = sqlExecute("SELECT DISTINCT " & CampoTabla & "  FROM " & Tabla & " WHERE " & CampoTabla & " <> ''  AND NOT " & CampoTabla & " IS NULL", "Cafeteria")
                ...
                Case "inchr"
                    ...
                    dtIncluyendo = sqlExecute("SELECT DISTINCT TOP 1000 " & CampoTabla & "  FROM " & Tabla & " WHERE " & CampoTabla & " <> ''  AND NOT " & CampoTabla & " IS NULL", "Cafeteria")
    ...
    Private Function FiltrarDatos(Texto As String, Excepcion As Boolean) As DataTable
        ...
        End If
        dtValores = sqlExecute("SELECT DISTINCT " & CampoTabla & "  FROM " & Tabla & " WHERE " & CampoTabla & " LIKE '%" & Texto & "%'", "Cafeteria")
        Return dtValores
    End Function
            ...
            Select Case Tipo
                Case "date"
                    If chkIncluyeFecha.Checked Then
                        Dim fecha2 As Date = FechaSQL(txtFecha2.Text)
                        valor = Campo & " >= '" & FechaSQL(txtFecha1.Text) & "' AND " & Campo & "<'" & FechaSQL(fecha2.AddDays(1).ToString()) & "'"
                    ElseIf chkExceptoFecha.Checked Then
                        Dim fecha2 As Date = FechaSQL(txtExceptoFecha2.Text)
                        valor = "NOT (" & Campo & " >= '" & FechaSQL(txtExceptoFecha1.Text) & "' AND " & Campo & "<'" & FechaSQL(fecha2.AddDays(1).ToString()) & "')"
    ...
    Private Sub ListarResultado(valor As String)
        ...
        Try
                ...
                dtResultado = dtResultadoCafeteria.Clone
                For Each dRow In dtResultadoCafeteria.Select(valor)
